...
class AttemptCountTest {
    ...
    void from_string_throws_on_null_or_blank() {
        assertThatThrownBy(() -> AttemptCount.from(null))
                .isInstanceOf(IllegalArgumentException.class);

        assertThatThrownBy(() -> AttemptCount.from(""))
                .isInstanceOf(IllegalArgumentException.class);
    ...
    void from_string_throws_when_not_number() {
        assertThatThrownBy(() -> AttemptCount.from("abc"))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> AttemptCount.from("1a"))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> AttemptCount.from("-2"))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> AttemptCount.from("+3"))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void from_string_accepts_spaces_and_parses_number() {
        assertThat(AttemptCount.from(" 5 ").value()).isEqualTo(5);
        assertThat(AttemptCount.from("01").value()).isEqualTo(1);
    }
    ...
    void from_string_throws_when_zero_or_negative() {
        assertThatThrownBy(() -> AttemptCount.from("0"))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> AttemptCount.from("-1"))
                .isInstanceOf(IllegalArgumentException.class);
    }
}